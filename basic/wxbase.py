...
class wx_minapp_login():
    def __init__(self,appid,secret):
        self.appid=appid
        self.secret=secret
        #self.codeurl='https://api.weixin.qq.com/sns/jscode2session?appid='+self.appid+'&secret='+self.secret+'&js_code=JSCODE&grant_type=authorization_code'
    def get_session_info(self,code=None):
        session_key_url=self.codeurl='https://api.weixin.qq.com/sns/jscode2session?appid='+self.appid+'&secret='+self.secret+'&js_code='+code+'&grant_type=authorization_code'
        try:
            response = requests.get(session_key_url, timeout=2)
        except (ConnectTimeout, ReadTimeout):
            raise ConnectionError('timeout', 'Connect timeout')
...
class WXBizDataCrypt:
    ...
    def decrypt(self, encrypted_data, iv):
        ...
        decrypted = simplejson.loads(self._unpad(cipher.decrypt(encrypted_data)))
...
class WxPay(object):
    ...
    def fetch(self, url, data):
        req = urllib.request.Request(url, data=self.to_xml(data).encode('utf-8'))
        try:
