...
#             pass
    ...
    def get_local_data(self , pk):
        ...
        List = []
        sql_list = """
            SELECT t.id,t.cname,t.tel, t.memo memo ,to_char(t.ctime,'YYYY-MM-DD') ctime,t.cstatus FROM tpmanager AS t
            WHERE t.v_code = '%s'
        """%pk
        if qqid != '':
            sql_list+= """
                and cast(id as varchar)+coalesce(t.cname,'')+cast(coalesce(t.tel,'') as varchar) like '%%%s%%'
            """%qqid
