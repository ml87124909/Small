...
class chome_dl(cBASE_DL):
    ...
    def sync_data_2(self,syncid):#A001 2
        ...
        l, t = self.db.select(sql,[syncid])
        if t > 0:
            self.db.query("delete from shopconfig where usr_id=%s and usr_id!=%s" ,[self.usr_id,syncid])
            for i in l:
    ...
    def sync_data_3(self,syncid):  # A001 3
        ...
        if t > 0:
            ...
            if tt>0:
                self.db.query("delete from logistics_way where usr_id=%s and usr_id!=%s", [self.usr_id,syncid])
                for i in ll:
    ...
    def sync_data_4(self,syncid):  # A001 5
        ...
        if tt>0:
            self.db.query("delete from hy_up_level where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in ll:
    ...
    def sync_data_6(self,syncid):  # C001
        ...
        l,t=self.db.select(sql,[syncid])
        if t>0:
            self.db.query("delete from advertis where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in l:
            ...
            if tt>0:
                self.db.query("delete from banner where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
                for j in ll:
    ...
    def sync_data_7(self,syncid):  # D001
        ...
        sql = """
           select id,name,type,pid,pic_icon,pic_imgs,paixu,remark,level 
           from category where usr_id=%s and COALESCE(del_flag,0)=0
           """
        l,t = self.db.select(sql,[syncid])
        if t>0:
            self.db.query("delete from category where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in l:
            ...
            sql = """
               select id,cp_id 
               from category where usr_id=%s and COALESCE(del_flag,0)=0 and level=1
               """
            ll, tt = self.db.select(sql,self.usr_id)
            for j in ll:
                did, cp_id=j
    ...
    def sync_data_8(self,syncid):  # D002
        ...
        l,t=self.db.select(sql,[syncid])
        if t>0:
            self.db.query("delete from spec where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in l:
            ...
            ll,tt=self.db.select(sql,[syncid])
            if tt>0:
                self.db.query("delete from spec_child where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
                for j in ll:
    ...
    def sync_data_9(self,syncid):  # D003
        ...
        l,t=self.db.select(sql,[syncid])
        if t>0:
            self.db.query("delete from goods_info where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in l:
            ...
            if tt>0:
                self.db.query("delete from goods_pics where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
                for j in ll:
                    did, copy_id=j
                    ...
                    if iN1>0:
                        self.db.query("delete from alone_discount where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
                        for m in lT1:
    ...
    def sync_data_11(self,syncid):  # E002
        ...
        l,t=self.db.select(sql,[syncid])
        if t>0:
            self.db.query("delete from score_conf where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
        ...
        if t>0:
            self.db.query("delete from score_set where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in l:
                days, score=i
    ...
    def sync_data_12(self,syncid):  # G001
        ...
        sql="""
            select add_money,giving from gifts where usr_id=%s
        """
        l,t=self.db.select(sql,[syncid])
        if t>0:
            self.db.query("delete from gifts where usr_id=%s and usr_id!=%s",[self.usr_id,syncid])
            for i in l:
                add_money, giving=i
                sql = """
    ...
    def buy_vip_data(self):
        ...
        try:
            ...
            sql = "select id,out_trade_no from pingtai_paylog where coalesce(pay_status,0)=0 and usr_id=%s"
            lT, iN = self.db.select(sql, [self.usr_id])
            if iN == 0:
                timeStamp = time.time()
                ...
                out_trade_no = 'A' + danhao[2:] + romcode
                data['usr_id'] = self.usr_id
            ...
            base_url = self.oTOLL['domain_url']
