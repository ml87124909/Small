...
class cE005_dl(cBASE_DL):
    def init_data(self):
        self.GNL = ['提现金额','用户余额','提现时间','用户ID','联系方式','用户名']
    ...
    def mRight(self):
        ...
        sql = u"""
            select w.del_money,round(u.balance::numeric ,2),to_char(w.ctime,'YYYY-MM-DD HH24:MI'),w.wechat_user_id,w.phone,u.cname 
                from withdraw_cash w 
                left join wechat_mall_user u on u.id=w.wechat_user_id and u.usr_id =w.usr_id
                where w.usr_id =%s order by w.ctime
        """%self.usr_id_p
    ...
    def local_add_save(self):
        ...
        self.db.update('users' , data , " usr_id = %s " % self.usr_id_p)
        dR['pk'] = self.usr_id
