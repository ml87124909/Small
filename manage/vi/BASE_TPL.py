...
from manage.vi.mselect import mselect_forGoods_Info, mselect_forGoods_Infos, mselect_forMList, mselect_forMList_mul, \
...
class cBASE_TPL(cVI_BASE):
    ...
    def Goods_mselect(self):
        ...
        mSelect.sUrl = 'manage?viewid=home&part=ajax&action=goods'
        mSelect.confirmjs = '''
                datas = sData.split("###");
    ...
    def Goods_mselect_mul(self):
        ...
        TmSelect.sUrl = 'manage?viewid=home&part=ajax&action=getGoods'
        TmSelect.confirmjs = '''
        if(sData != ''){
    ...
    def show_ticket(self):
        ...
        mSelect.sUrl = 'manage?viewid=home&part=ajax&action=ticket'
    ...
    def Goods_mselects(self):
        mSelect = mselect_forGoods_Infos('Goods', ['ID', '商品名称'], '拼团商品列表',
                                         wh=[500, 300], dnl=[0, 1, 2, 3], search_holder='请输入商品名称')
        mSelect.sUrl = 'manage?viewid=home&part=ajax&action=pt_goods'
        mSelect.confirmjs = '''
                datas = sData.split("###");
                $('input[name="goods_id"]').val(datas[0]);
                $('input[name="good_name"]').val(datas[1]);
    ...
    def Spec_mselect_child(self):  # 自定方法
        ...
        TmSelect = mselect_forMList_spec('spec_mul', viewid=self.viewid, nl=nL, wh=[900, 400], title='选择规格型号', dnl=dnl,
                                         canjp='')
        TmSelect.sUrl = 'manage?viewid=C004&part=ajax&action=getSpecChild'
        TmSelect.confirmjs = '''
        if(sData != ''){
            ...
            $.ajax({
                url:"manage?viewid=C004&part=set_spec_child&sid="+spec_types+"&spec_cids="+sData,
                type:"post",
                async:false,
    ...
    def Hot_mselects(self):
        mSelect = mselect_forGoods_Infos('Goods', ['ID', '商品名称'], '商品列表',
                                         wh=[500, 300], dnl=[0, 1, 2, 3], search_holder='请输入商品名称')
        mSelect.sUrl = 'manage?viewid=C006&part=ajax&action=hot_goods'
        mSelect.confirmjs = '''
                datas = sData.split("###");
                $('input[name="goods_id"]').val(datas[0]);
                $('input[name="good_name"]').val(datas[1]);
