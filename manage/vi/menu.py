...
class cmenu(cBASE_TPL):
    ...
    def goPartRefresh(self):
        ...
        return self.mScriptMsg("菜单数据已刷新",[['manage?viewid=menu','返回菜单设计']],'success')

    def goPartDelete(self):
        #WeEngine = engine.cEngine(self.account)
        self.WxApi.menuDelete()
        self.dl.cookie.isetcookie('menu_%s' % self.weid , '' , -3600)
        return self.mScriptMsg("菜单数据已删除",[['manage?viewid=menu','返回菜单设计']],'success')
    ...
    def goPartSaveMenu(self):
        ...
        if res == 0:
            ...
            #return self.mScriptMsg("菜单数据已保存",[['manage?viewid=menu','返回菜单设计']],'success')
        else:
            dR = {'code': '1', 'MSG': '保存失败!'}
            #return self.mScriptMsg("菜单数据保存失败，错误代码：%s[%s]" % (res,self.WxApi.weixin_code(res)) ,[['manage?viewid=menu','返回菜单设计']],'error')
            return self.jsons(dR)
