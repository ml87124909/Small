...
class cbasic(cBASE_TPL):
    ...
    def goPartLocalform(self):
        ...
        if id:
            ...
            if not rule['rule'] :
                return self.mScriptMsg('抱歉，您操作的规则不在存或是已经被删除！',[['manage?viewid=basic','返回列表']])
    ...
    def goPartPost(self):
        ...
        if pk:
            ...
            return self.mScriptMsg('规则操作成功！',[['manage?viewid=basic&part=localform&id=%s' % pk,'返回']],'success')
        else:
