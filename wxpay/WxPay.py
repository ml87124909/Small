...
from basic.publicw import db,oTOLL,oUSER,oGOODS_D,oGOODS_N,oPT_GOODS
#db,subid,xml_data,
class cWxPay:
    ...
    # def cPayNotify(db, subid, xml_data, oUSER, oGOODS_D, oGOODS_N, oPT_GOODS):
    #     try:
    #         # print_log(db, '微信支付回调', '6666666666')
    #         sql = "select usr_id from users where usr_id=%s "
    #         l, t = db.select(sql, subid)
    #         if t > 0:
    #
    #             try:
    ...
    #                         out_trade_no = data['out_trade_no']
    #                         me = int(data['total_fee']) / 100
    #                         print_log(db, 'wechat_mall_payment测试:',
    #                                   '%s,%s,%s' % (out_trade_no, data['total_fee'], subid))
    #                         sql = "select id,order_num,ctype from wechat_mall_payment where payment_number=%s and coalesce(status,0)=0 and total_fee=%s and usr_id=%s"
    #                         l, t = db.select(sql, [out_trade_no, data['total_fee'], subid])
    #                         # print_log(db, 'wechat_mall_payment测试:%s' % i,'%s,%s,%s' % (out_trade_no, data['total_fee'], subid))
    #                         if t == 0:
    #                             return 1
    #                         id, order_num, ctype = l[0]
    #                         if str(ctype) == '6':
    #                             sql = "select wechat_user_id from offline_pay where order_num=%s and usr_id=%s "
    #                             k, r = db.select(sql, [out_trade_no, subid])
    #                             if r == 0:
    #                                 return 1
    #                             try:
    #                                 uid = k[0][0]
    ...
    #                             }
    #
    #                             db.update("wechat_mall_payment", payment,
    #                                       "payment_number='%s' and usr_id=%s" % (out_trade_no, subid))
    #
    #                             score = 0
    #                             # 赠送积分
    #                             sql = "select COALESCE(integral,0),COALESCE(vip_integral,0) from score_conf where usr_id=%s"
    ...
    #
    #                                 sql = "update wechat_mall_user set score=COALESCE(score,0)+%s where usr_id=%s and id=%s"
    #                                 db.query(sql, [score, subid, uid])
    #                                 user_log(db, subid, uid, '扫码支付回调增加积分', 'score:%s' % score)
    #
    #                             sql = """update offline_pay set
    #                                 status=1,utime=now(),paytime=now(),score=%s where usr_id=%s and order_num=%s"""
    #                             db.query(sql, [score, subid, out_trade_no])
    #
    #                         elif str(ctype) == '5':
    #                             sql = "select id,wechat_user_id from top_up where order_no=%s and usr_id=%s "
    #                             k, r = db.select(sql, [out_trade_no, subid])
    ...
    #                                                                     values(%s,%s,%s,%s,%s,now())
    #                                                                 """
    #                             db.query(sql, [subid, 'wechat_mall_payment', 'payment_number:%s' % out_trade_no,
    #                                            '支付回调更新wechat_mall_payment表数据', subid])
    #
    #                             sqld = "update top_up set status=7,status_str='已完成',uid=0,utime=now() where order_no=%s"
    #                             db.query(sqld, out_trade_no)
    #                             real_money = float(me)
    #                             give = 0
    #                             sql = "select giving from  gifts where add_money<=%s and usr_id=%s order by add_money desc limit 1 "
    #                             lT, iN = db.select(sql, [me, subid])
    #                             if iN > 0:
    #                                 give = lT[0][0]
    #                                 real_money += float(give)
    #
    ...
    #                                 'ctype': '1',
    #                                 'ctype_str': '充值',
    #                                 'pay_ctime': getToday(9),
    #                                 'give': give
    #
    #                             }
    #                             db.insert('cash_log', cash_dict)
    #                             sql = "update wechat_mall_user set balance =coalesce(balance,0)+%s where id=%s"
    #                             db.query(sql, [real_money, uid])
    #                             oUSER.update(subid, uid)
    #
    #                             return 0
    #                         elif str(ctype) == '1':
    #                             sql = "select id,wechat_user_id from vip_member where order_no=%s and usr_id=%s "
    #                             lT, iN = db.select(sql, [out_trade_no, subid])
    #                             if iN == 0:
    #                                 return 1
    #                             try:
    #                                 uid = lT[0][1]
    ...
    #
    #                                 sqlh = "update wechat_mall_user set hy_etime=%s where usr_id =%s and id=%s"
    #                                 db.query(sqlh, [hy_etime, subid, uid])
    #                                 sql = """insert into wechat_mall_order_log(usr_id,edit_name,edit_memo,edit_remark,cid,ctime)
    #                                                                         values(%s,%s,%s,%s,%s,now())
    #                                                                     """
    #                                 db.query(sql, [subid, 'wechat_mall_user', "id:%s,hy_flag=1" % uid,
    #                                                '会员服务支付回调更新wechat_mall_user表数据', subid])
    #
    #                                 oUSER.update(subid, uid)
    #                                 return 0
    #
    ...
    #
    #                             mn = "select id,wechat_user_id from wechat_mall_order where  order_num=%s and usr_id=%s"
    #                             m, n = db.select(mn, [order_num, subid])
    #
    #                             if n == 0:
    #                                 return 1
    #
    ...
    #
    #                             sqlo = "update wechat_mall_order set status=2,status_str='待发货',pay_ctime=now(),uid=0,utime=now() where order_num=%s"
    #                             db.query(sqlo, order_num)
    #                             sqld = "update wechat_mall_order_detail set status=2,status_str='待发货',uid=0,utime=now() where order_num=%s"
    #                             db.query(sqld, order_num)
    #                             sql = """insert into wechat_mall_order_log(usr_id,edit_name,edit_memo,edit_remark,cid,ctime)
    #                                                                 values(%s,%s,%s,%s,%s,now())
    #                                                             """
    #                             db.query(sql,
    #                                      [subid, 'wechat_mall_order',
    #                                       "order_num:%s,status=2,status_str='待发货'" % order_num,
    #                                       '支付回调更新wechat_mall_order表数据', subid])
    #                             sqlt = """select id,wechat_user_id,ctype,kuaid,balance,ptkid,ptid,pt_type,phone
    #                                 from wechat_mall_order where order_num=%s and usr_id=%s """
    #                             ll, tt = db.select(sqlt, [order_num, subid])
    #                             if tt > 0:
    #                                 order_id, wechat_user_id, ctype, kuaid, balance, ptkid, ptid, pt_type, phone = ll[0]
    #                                 if float(balance) > 0:
    #                                     pay_status = 3
    #                                     pay_status_str = '组合支付'
    #                                 else:
    #                                     pay_status = 1
    #                                     pay_status_str = '微信支付'
    #                                 sqlo = "update wechat_mall_order set pay_status=%s,pay_status_str=%s,pay_ctime=now(),uid=0,utime=now() where order_num=%s"
    #                                 db.query(sqlo, [pay_status, pay_status_str, order_num])
    #                                 sql = """insert into wechat_mall_order_log(usr_id,edit_name,edit_memo,edit_remark,cid,ctime)
    #                                                                                             values(%s,%s,%s,%s,%s,now())
    #                                                                                         """
    #                                 db.query(sql, [subid, 'wechat_mall_order',
    #                                                "order_num:%s,pay_status=%s,pay_status_str=%s" % (
    #                                                    order_num, pay_status, pay_status_str),
    #                                                '支付回调更新wechat_mall_order表数据', subid])
    #
    #                                 if str(kuaid) == '1' and str(ctype) != '1':  # 上门自提
    #                                     sqlo = "update wechat_mall_order set status=4,status_str='待自提',uid=0,utime=now() where order_num=%s"
    #                                     db.query(sqlo, order_num)
    #                                     sql = """insert into wechat_mall_order_log(usr_id,edit_name,edit_memo,edit_remark,cid,ctime)
    #                                                                                                                                 values(%s,%s,%s,%s,%s,now())
    #                                                                                                                             """
    ...
    #                                                    '支付回调更新wechat_mall_order表数据', subid])
    #                                 elif str(kuaid) == '2' and str(ctype) != '1':  # 无需配送
    #                                     sqlo = "update wechat_mall_order set status=6,status_str='待评价',uid=0,utime=now() where order_num=%s"
    #                                     db.query(sqlo, order_num)
    #                                     sql = """insert into wechat_mall_order_log(usr_id,edit_name,edit_memo,edit_remark,cid,ctime)
    #                                                                                                                                 values(%s,%s,%s,%s,%s,now())
    #                                                                                                                             """
    ...
    #                                                    '支付回调更新wechat_mall_order表数据', subid])
    #
    #                                 sql = "select good_id,amount,inviter_user,good_name,cid from wechat_mall_order_detail  where usr_id=%s and order_num=%s "
    #                                 lll, t = db.select(sql, [subid, order_num])
    #                                 for ii in lll:
    #                                     good_id, amountm, user, good_name, cid = ii
    #                                     oGOODS_D.updates(subid, good_id, amountm)
    #                                     oGOODS_N.update(subid, good_id)
    #                                     if str(user) != '0' and str(user) != str(cid):  # 下单返现
    #                                         good_D = oGOODS_D.get(subid, int(good_id))
    #                                         if good_D != {}:
    #                                             shareInfo = good_D['shareInfo']
    #                                             basicInfo = good_D['basicInfo']
    #                                             share_type = shareInfo['share_type']
    ...
    #                                                     where usr_id=%s and id=%s
    #                                                     """
    #                                                     db.query(sql, [float(share_number), subid, user])
    #                                                     sqlg = "insert into user_log(usr_id,wechat_user_id,cname,memo,ctime)values(%s,%s,'balance',%s,now())"
    #                                                     db.query(sqlg, [subid, user, '%s下单返现给%s' % (cid, user)])
    #                                                     sql = """
    #                                                     insert into cash_log(usr_id,wechat_user_id,ctype,ctype_str,change_money,
    #                                                     typeid,typeid_str,remark,goods_id,goods_name,cid,ctime)
    #                                                     values(%s,%s,2,'返现',%s,1,'分享返现','单次分享返现返',%s,%s,%s,now())
    #                                                     """
    #                                                     parm = [subid, user, float(share_number), good_id, gname, cid]
    #                                                     db.query(sql, parm)
    #                                                     sql = """
    #                                                         insert into profit_record(usr_id,wechat_user_id,ctype,ctype_str,
    #                                                         share_type,
    ...
    #                                                     db.query(sqlg, [subid, user, '%s下单返积分给%s' % (cid, user)])
    #                                                     sql = """
    #                                                             insert into integral_log(usr_id,wechat_user_id,type,typestr,in_out,
    #                                                             inoutstr,amount,cid,ctime)values(%s,%s,7,'分享返',0,'收入',%s,%s,now())
    #                                                             """
    #                                                     parm = [subid, good_id, float(share_number), cid]
    #                                                     db.query(sql, parm)
    #                                                     sql = """insert into profit_record(usr_id,wechat_user_id,ctype,
    #                                                     ctype_str,share_type,
    #                                                         share_type_str,change_money,goods_id,goods_name,cid,ctime)
    ...
    #                                                     where usr_id=%s and COALESCE(del_flag,0)=0 and id=%s
    #                                                                     """
    #                                                     parm = [subid, ticket_id]
    #                                                     l, n = db.select(sql, parm)
    #                                                     if n > 0:
    #                                                         now_, max_num, dateend, total, cname, remark, type_id, type_str, type_ext = \
    #                                                             l[0][0:9]
    #                                                         apply_id, apply_str, apply_ext_num, apply_ext_money, apply_goods_id, use_time, use_time_str = \
    ...
    #                                                             values(%s,%s,2,'优惠券收益',5,'好友下单返优惠券',%s,%s,%s,%s,now(),%s)
    #                                                                 """
    #                                                             parm = [subid, user, change_money, good_id, gname, cid,
    #                                                                     ticket_id]
    #                                                             db.query(sql, parm)
    #                                                 oUSER.update(subid, user)
    #                                 if str(ctype) == '2':  # 支付更新团的状态gid
    #                                     if str(pt_type) == '0':  # 开团
    #                                         Pingtuan_add(db, subid, wechat_user_id, ptid, order_id, oUSER, oPT_GOODS,
    #                                                      phone)
    #                                     if str(pt_type) == '1':  # 参团
    #                                         Pingtuan_join(db, subid, wechat_user_id, ptkid, order_id, oUSER, oPT_GOODS,
    #                                                       phone)
    #                             oUSER.update(subid, uid)
    #                     except:
    #                         errstr = str(traceback.format_exc())
    #                         print_log(db, '支付回调报错具体信息', '%s' % errstr)
    #                         return 1
    #                     return 0
    #                 else:
    #                     try:
    ...
    #                             'utime': getToday(9)
    #                         }
    #                         db.update("wechat_mall_payment", datas, "payment_number='%s'" % data['out_trade_no'])
    #
    #                     except:
    #                         pass
    #                     return 1
