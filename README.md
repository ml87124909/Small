...
> 在贡献代码、功能(插件)、小程序模板的前提,不禁止任何形式的使用、修改、发布、分发该项目。

> 在不贡献代码、功能(插件)、小程序模板的情况下，不禁止以学习为目的的使用、修改、发布、分发该项目。

> 本是没有打算收取任何商业授权费用，而几个采用本项目进行商用的兄弟都担心后续授权问题，怕万一要收取授权费用会很高而影响他们的运营。
> 我将商业授权费用明确下来并固定，以后不会进行变更，让商用的兄弟可以放下担忧。
> 各版本商业授权的费用：个人版本500元，SAAS版本1000元，B2B2C版本1000元。
> 商业授权费用包含愿意合并到本项目的接口定制费用，不愿意合并到本项目的接口定制不承接。
