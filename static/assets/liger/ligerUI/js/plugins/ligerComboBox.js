...
(function ($)
{
    ...
    $.ligerui.controls.ComboBox.ligerExtend($.ligerui.controls.Input, {
        ...
        _render: function ()
        {
            ...
            if (p.absolute)
                g.selectBox.appendTo('body').addClass("l-box-select-absolute select"+p.valueFieldID);
            else
        ...
        {	
            ...
            var contain = function (checkvalue)
            {
                ...
                {	
                    if (targetdata[i] == checkvalue) return true; //&& targetdata[i] != ""修改排除‘’不取默认值
                }
                return false;
            };
        ...
        treeSelectInit: function (value)
        {
            ...
            { 
                ...
                $(valuelist).each(function (i, item)
                {
					...
					if (g.treeManager.getTextByID(item) != null )
					{
                ...
                success: function (result)
{
        ...
        setSelectNew: function ()
        {
            var g = this, p = this.options;
            this.clearContent();
            $("option",g.select).each(function (i){
                var text=g.lastInputText;
                var val = $(this).val();
                var txt = $(this).html();
            ...
            g.inputText.keyup(function (event)
            {
                ...
                { 
                    ...
                    } else if (p.grid)
                    {
                    	...
                    	//自动完成兼容使用data数据 mod by Mast 20161205
                    	var text=g.lastInputText;
                    	g.setParm('key', currentKey);
                        ...
                        //自动完成兼容使用直接绑定html select对象 mod by Mast 20161205
                        g.setSelectNew();
                        g._selectBoxShow();
                    }
