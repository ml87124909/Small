...
* 
* modify: 增加data传入isselected 判断是否selected中 -- MINZ
*/
(function ($)
{
    ...
    $.ligerui.controls.Tree.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            g.sysAttribute = ['isexpand', 'ischecked', 'href', 'style', 'delay','isselected'];
        ...
        _getTreeHTMLByData: function (data, outlineLevel, isLast, isExpand)
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                } else
                {
					var ztclass = ''
					var bz = ''
					if (typeof(o.sh_flag) != "undefined")
					{	if (o.sh_flag == 1)
						{ ztclass = 'color:#0000bf;'
						  bz = '(已审)'
						}
						else if (o.sh_flag == 2)
						{ ztclass = 'color:#ff7f00;'
						  bz = '(反审)'
						}
						else if (o.sh_flag == 3)
						{ ztclass = 'color:#ff0000;'
						  bz = '(已删)'
						}
						else{
						  ztclass = ''
                    ...
                    treehtmlarr.push('<span'+' style="'+ztclass+'">' + o[p.textFieldName]+bz+ '</span>');//class="" style="color:#ff0000;"
                }
                treehtmlarr.push('</div>');
                if (g.hasChildren(o))
                {
